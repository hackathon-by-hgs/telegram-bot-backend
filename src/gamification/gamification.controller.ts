...
export class GamificationController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns `UserStats` (xp, level, streak, badges, lastActiveAt). Returns 200 with `null` if the user has no stats row yet.',
  })
  @ApiOkResponse({
    type: UserStatsDto,
  ...
  addXp(@Body() dto: AddXpDto) {
    return this.gamification.addXp(
      dto.userId,
      Number(dto.amount) || 0,
      'manual',
    );
  ...
  @ApiOperation({
    ...
    description:
      'Top users by XP (descending), ties broken by streak. Each row includes a thin user summary (`username`, `referralCode`).',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
