...
export class WalletService {
  ...
  async analyze(address: string, chain = 'eth'): Promise<WalletAnalysis> {
    ...
    if (dangerous_approvals.length) {
      risk_indicators.push(`${dangerous_approvals.length} unlimited approval(s) outstanding`);
    }
    ...
    if (wallet_health_score < 60) {
      this.events.emit(EVENTS.WALLET_RISK_DETECTED, { address, score: wallet_health_score });
    }
    ...
    return {
      ...
      recommendations: this.recommend(dangerous_approvals.length, suspicious_contracts.length),
    };
  }

  private recommend(approvals: number, badContracts: number) {
    const r: string[] = [];
    if (approvals > 0) r.push('Revoke unlimited token approvals via revoke.cash');
    if (badContracts > 0) r.push('Move funds to a fresh wallet — current address has been exposed to flagged contracts');
    if (r.length === 0) r.push('Wallet appears clean. Continue practicing good approval hygiene.');
    return r;
  }
  ...
  private async fetchApprovals(address: string, _chain: string): Promise<Array<{ token: string; spender: string; value: 'max' | string }>> {
    ...
    try {
      const { data } = await firstValueFrom(
        this.http.get(`https://deep-index.moralis.io/api/v2.2/wallets/${address}/approvals`, {
      ...
      return list.map((row) => ({
        ...
        value: BigInt(row.value ?? 0) >= MAX_UINT256 ? 'max' : String(row.value ?? 0),
      }));
    } catch (err) {
      this.log.warn(`moralis approvals fetch failed: ${(err as Error).message}`);
      return [];
    }
  }
}
