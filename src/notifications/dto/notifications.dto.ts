...
export class SendNotificationDto {
  @ApiProperty({ description: 'Recipient user ID', example: 'ckxk7g2v90000abcd1234efgh' })
  @IsString()
  @IsNotEmpty()
  userId!: string;
  ...
  @ApiProperty({ description: 'Short headline (≤ 120 chars).', example: 'Bridge before midnight!', maxLength: 120 })
  @IsString()
  @MaxLength(120)
  title!: string;
...
export class SendNotificationResponseDto {
  @ApiProperty({ description: '`true` when both persistence and Telegram delivery succeeded.', example: true })
  ok!: boolean;

  @ApiPropertyOptional({ description: 'Notification row ID.', example: 'ckxk7g2v90400abcd1234efgh' })
  id?: string;
